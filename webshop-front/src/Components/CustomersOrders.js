...
const CustomersOrders = () => {
  ...
  const fetchOrderHistory = async () => {
    try {
      axios.defaults.headers.common['Authorization'] = `Bearer ${JSON.parse(sessionStorage["Token"])}`;
      await axios.get('https://localhost:7042/api/Orders/getcustomersorder/' + userID);
    } catch (error) {
  ...
  useEffect(() => {
    const fetchData = async (orderId) => {
      try {
        const response = await axios.get(`https://localhost:7042/api/Product/getproductbyorderid/${orderId}`);
